...
def change_login(request):
    """Смена логина с подтверждением пароля"""
    if 'user_id' not in request.session:
        messages.error(request, 'Сначала войдите в систему')
        return redirect('/login/')
    ...
    if request.method == 'POST':
        ...
        if User.objects.filter(login=new_login).exclude(user_id=user.user_id).exists():
        ...
        messages.success(request, f'Логин успешно изменён на {new_login}')
        return redirect('/profile/')
...
def delete_track(request, track_id):
    """Удаление трека (только владелец или админ)"""
    if 'user_id' not in request.session:
        return JsonResponse({'error': 'Необходимо авторизоваться'}, status=401)
    ...
    if track.user.user_id != user_id and user_role != 'admin':
        return JsonResponse({'error': 'У вас нет прав на удаление этого трека'}, status=403)
    ...
    if request.method == 'POST':
        try:
            ...
            track.delete()
            return JsonResponse({'success': True, 'message': 'Трек удалён'})
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=500)
